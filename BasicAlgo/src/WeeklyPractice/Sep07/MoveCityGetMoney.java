...
public class MoveCityGetMoney {
    public static int maxProfits1(int n, int m, int k, int[] c, int[] a, int[] b) {
        int[][] dp = new int[n][m];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                dp[i][j] = -1;
        ...
        return process(k, 0, c, a, b, dp, m);
    }

    public static int process(int cur, int index, int[] c, int[] a, int[] b, int[][] dp, int m) {
        if (index == m) {
            return 0;
        }
        if (dp[cur][index] != -1) {
        ...
        // 可能性1
        int p1 = process(cur, index + 1, c, a, b, dp, m);
        int p2 = 0;
        if (cur == c[index]) {
            p2 = a[index] + process(cur, index + 1, c, a, b, dp, m);
        } else {
            p2 = b[index] + process(c[index], index + 1, c, a, b, dp, m);
        }
        dp[cur][index] = Math.max(p1, p2);
        return dp[cur][index];
    ...
    public static int maxProfit2(int n, int m, int k, int[] c, int[] a, int[] b) {
        ...
        int ans = 0;
        for (int i = 0; i < m; i++) {
            int p1 = Math.max(st.getMax(0, c[i] - 1), st.getMax(c[i] + 1, n - 1)) + b[i];
            int p2 = st.getMax(c[i], c[i]) + a[i];
    ...
    public static class SegmentTree {
        ...
        public SegmentTree(int len) {
            SIZE = len;
            max = new int[(SIZE + 1) << 2];
            Arrays.fill(max, Integer.MIN_VALUE);
        }

        public void update(int index, int c) {
            index++;
            update(index, index, c, 1, SIZE, 1);
        }
        ...
        public int getMax(int l, int r) {
            ...
            return getMax(l, r, 1, SIZE, 1);
        }

        public int getMax(int L, int R, int l, int r, int rt) {
            if (L <= l && r <= R) {
                return max[rt];
            }
            int mid = (r + l) >> 1;
            ...
            if (L <= mid) {
                ans = getMax(L, R, l, mid, rt << 1);
            }
            if (R > mid) {
                ans = Math.max(ans, getMax(L, R, mid + 1, r, rt << 1 | 1));
            }
            return ans;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTimes; i++) {
            ...
            int ans1 = maxProfits1(n, m, k, c, a, b);
