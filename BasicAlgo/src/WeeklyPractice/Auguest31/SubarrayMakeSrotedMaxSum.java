...
public class SubarrayMakeSrotedMaxSum {
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (size > 0 && curVal > 0) {
                ...
                if (leftVal >= curVal) {
                    ...
                    if (idxDiff <= valDiff) {
                        dp[i] += sum(idxDiff, curVal) + dp[leftIdx];
                        curVal = 0;
                        curIdx = 0;
                        break;
                    } else {
                        dp[i] += sum(idxDiff, curVal);
                        curVal -= idxDiff;
                        curIdx = leftIdx;
                        size--;
                    }
    ...
    public static long sum(int n, int max) {
        n = Math.min(n, max);
